...
public partial class Login : System.Web.UI.Page
{
    ...
    protected void imgBtnLogin_Click(object sender, ImageClickEventArgs e)
    {
        ...
        string[] strMsg = oBll.CheckLogin().Split('-');//Splitting the valid login status and employee code to use it furhter
        oBll.GetEmail();
        if (oBll.oDsEmail.Tables[0].Rows.Count > 0)
        {
            ...
            {
                string[] strMsg1 = oBll.CheckVendorLogin().Split(',');//Checking if it's a vendor login
                string[] strMsg2 = strMsg1[0].Split('-');//Splitting the valid login status and vendor code to use it furhter
                if (strMsg2[0] == "valid")
                {
                    lblMsg.Text = "";
                    Session["UserID"] = strMsg2[1];//Storing the vendor id in session to be further used
                    Session["VendID"] = strMsg1[1];//Storing the vendor's email id in session to be further used
    ...
    //Code to send password to employee if they forget
    protected void btnForgotPass_CLick(object sender, EventArgs e)
    {
        try
        {
            ...
            if (oBll.oDsEmail.Tables[0].Rows.Count > 0)
            {
                ...
                Sendmail(strFromEmailID,strPass, strToEmailID,strSmtpHost,iSmtpPort);//Invoking send email method to send password
            }
        }    
    ...
    #region "Methods"
    //Send email method to send password to the employee
    ...
    //Method to generate random password,,,right now not being invoked
